...
const bestMatchingSet = (handOne, handTwo, match) => {
    return [handOne, handTwo].reduce((highest, current) => {
        return getCardValueByCount(current, match) > getCardValueByCount(highest, match) ? current : highest;
    });
};
...
const bestStraightOrFlush = (handOne, handTwo) => {
    return [handOne, handTwo].reduce((highest, current) => {
        ...
        const highestA = orderValues(highest.cards)[highest.cards.length -1];
        const highestB = orderValues(current.cards)[current.cards.length -1];
        if (highestA === highestB) {
            return 'Equal hands!';
        }
        return highestA > highestB ? highest : current;
    });
};
